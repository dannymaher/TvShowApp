...
import uk.ac.tees.tvshowapp.fragments.TVShowDetailsFragment;
...
public class TestTVShowDetailsFragment {
    ...
    public void setupFragment() {
        ...
        args.putString("title", exampleShow.getName());

        FragmentScenario.launchInContainer(TVShowDetailsFragment.class, args);
    }
    ...
    public void titleCorrect() {
        onView(withId(R.id.show_name))
                .check(matches(withText(exampleShow.getName())));
    }

    /**
     * check the dates is displayed correctly
     */
    @Test
    public void datesCorrect() {
        onView(withId(R.id.show_dates))
                .check(matches(withText(exampleShow.getDateString())));
    }

    /**
     * check the overview is displayed correctly
     */
    @Test
    public void overviewCorrect(){
        onView(withId(R.id.show_overview))
                .check(matches(withText(exampleShow.getOverview())));
    }

    /**
     * check the next episode / last episode text is correct
     */
    @Test
    public void nextEpisodeLastEpisode_labelCorrect(){
        onView(withId(R.id.next_episode_text))
                .check(matches(withText(R.string.last_episode)));
    }

    /**
     * check the last episode's details are displayed correctly.
     */
    @Test
    public void lastEpisode_textCorrect(){
        onView(withId(R.id.episode_name))
                .check(matches(withText(exampleShow.getLastEpisodeToAir().getName())));

        onView(withId(R.id.episode_overview))
                .check(matches(withText(exampleShow.getLastEpisodeToAir().getOverview())));
    }

    /**
     * test the current season's details are displayed correctly
     */
    @Test
    public void currentSeason_textCorrect(){
        Season lastSeason = exampleShow.getSeasons().get(exampleShow.getSeasons().size() - 1);

        onView(withId(R.id.season_name))
                .check(matches(withText(lastSeason.getName())));

        onView(withId(R.id.season_overview))
                .check(matches(withText(lastSeason.getOverview())));
    }

    /**
     * test genres are displayed correcly
     */
    @Test
    public void genresCorrect(){
        for(Genre genre: exampleShow.getGenres()){
    ...
    /**
     * test show cast is displayed correctly
     */
    @Test
    public void castCorrect(){
        Credits credits = TmdbMock.getCredits();

        onView(withId(R.id.cast_show_container))
                .perform(scrollTo());
